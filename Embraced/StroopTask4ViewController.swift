//
//  StroopTask4ViewController.swift
//  Embraced
//
...
class StroopTask4ViewController: ActiveStepViewController {
            ...
            FirebaseStorageManager.shared.externalStorage(filePath: filePath, fileUrl: recordedAudioURL) { (uploadTask, error) in
                ...
                if let task = uploadTask {
                    task.observe(.success, handler: { (snapshot) in
                        StroopModel.shared.file_4 = filePath
                        StroopModel.shared.rt_4 = self.reactionTime
...
extension StroopTask4ViewController: AVAudioRecorderDelegate {
