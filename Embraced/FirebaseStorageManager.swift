...
class FirebaseStorageManager {
    ...
    func addDataToDocument(payload: [String: Any]) {
        if let id = pid {
            db.collection(collectionName).document(id).setData(payload) { (err) in
                if let err = err {
                    print("Error: \(err)")
    ...
    func addFileToStorage(name: String, path: URL, test: String) {
        if let id = pid {
            let storage = Storage.storage()
            let storageRef = storage.reference()
            let participantRef = storageRef.child("\(id)/\(test)/\(name)")
