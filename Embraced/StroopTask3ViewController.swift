//
//  StroopTask3ViewController.swift
//  Embraced
//
...
class StroopTask3ViewController: ActiveStepViewController {
    ...
    @objc func moveOn() {
//        self.performSegue(withIdentifier: "moveToPreTask4", sender: nil)
        let vc = StroopPreTask4ViewController()
            ...
            FirebaseStorageManager.shared.externalStorage(filePath: filePath, fileUrl: recordedAudioURL) { (uploadTask, error) in
                ...
                if let task = uploadTask {
                    task.observe(.success, handler: { (snapshot) in
                        StroopModel.shared.file_3 = filePath
...
extension StroopTask3ViewController: AVAudioRecorderDelegate {
