//
//  MotorTask3ViewController.swift
//  Embraced
//
...
class MotorTask3ViewController: ActiveStepViewController {

    let TEST_NAME = "Motor"
    var documentPath: URL?
    let fileName = "task3.png"
    ...
    override func viewDidLoad() {
        ...
        instructionsLabel.text = "motor_dominant".localized(lang: language)
        ...
        canvas.image = Utility.getImage(path: "motorTask/\(DataManager.sharedInstance.motorTask[0])")
    ...
    @objc func showMoreActions(touch: UITapGestureRecognizer) {
        ...
        let dynamicView = MotorTapView(frame: CGRect(x: touchPoint.x - 10, y: touchPoint.y - 10, width: 20, height: 20))
        dynamicView.indexLabel.text = "\(tapIndex)"
        tapViews.append(dynamicView)
        self.canvas.addSubview(dynamicView)
            ...
            FirebaseStorageManager.shared.externalStorage(filePath: filePath, fileUrl: saveImagePath) { (uploadTask, error) in
                ...
                if let task = uploadTask {
                    task.observe(.success, handler: { (snapshot) in
                        MotorModel.shared.dominants["2"] = filePath
                        
                        FirebaseStorageManager.shared.addDataToDocument(payload: [
                            "motorTask": MotorModel.shared.getModel()
                        ])
                        
                        let vc = MotorTask4ViewController()
