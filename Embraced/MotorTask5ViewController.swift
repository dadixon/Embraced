//
//  MotorTask5ViewController.swift
//  Embraced
//
...
class MotorTask5ViewController: ActiveStepViewController {

    let TEST_NAME = "Motor"
    var documentPath: URL?
    let fileName = "task5.png"
    ...
    override func viewDidLoad() {
        ...
        canvas.image = UIImage(named: "Punteado")
            ...
            FirebaseStorageManager.shared.externalStorage(filePath: filePath, fileUrl: saveImagePath) { (uploadTask, error) in
                ...
                if let task = uploadTask {
                    task.observe(.success, handler: { (snapshot) in
                        MotorModel.shared.dominants["3"] = filePath
